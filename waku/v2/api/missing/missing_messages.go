...
// It receives some function callbacks that are used to determine whether a message exists or not, since that logic is application specific
func NewMissingMessageVerifier(store *store.WakuStore, messageExists func(pb.MessageHash) (bool, error), timesource timesource.Timesource, logger *zap.Logger, options ...MissingMessageVerifierOption) *MissingMessageVerifier {
...
func (m *MissingMessageVerifier) fetchMessagesBatch(c chan<- *protocol.Envelope, interest criteriaInterest, batchFrom int, batchTo int, now time.Time) error {
	...
	for !result.IsComplete() {
		for _, mkv := range result.Messages() {
			...
			exists, err := m.messageExists(hash)
			if err != nil {
				return err
			}
